...
include('include/utils.php');//For files manage

if (isset($_GET['fldr']) //Ayuda a generar vistas obtiene folder de almacenamiento
    && !empty($_GET['fldr'])
...
// add lang file to session for easy include CONFIGURACIONES DE IDIOMA
...
//Parametros qe pasaremos a dialog
...
<!--Configuraciones para DROPZONE-->
	...
	<script type="text/javascript">//Seleccionar JQUERY 
	...
	<script src="js/selectable.js" type="text/javascript"></script>
	
	<script src="js/jquery.contextMenu.min.js" type="text/javascript"></script>    
	<!--COnfigruarciones DROPZONE para subir archivos-->
	<script>
	    ...
	    //Jquery contextMenu configuraciones
	</script>
...
<div class="uploader">
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
				...
				<form action="dialogo.php" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
...
//Obtener el directorio de nuestros files
$files = scandir($current_path.$rfm_subfolder.$subdir);
$n_files=count($files);

//php sorting Ordenamiento de files	
$sorted=array();
...
?>          
<!-- header div start BARRA DE UPLOAD CREATE FOLDER-->
...
<!-- header div end -->
	...
	<!--Cuando subidos nuevos items-->
	<li class="pull-right"><a id="refresh" class="btn-small" href="dialogo.php?<?php echo $get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
		...
		foreach ($files as $file_array) {
			...
			?>
			    ...
			    ?>	<figure data-name="<?php echo $file ?>" class="<?php if($file=="..") echo "back-"; ?>directory" data-type="<?php if($file!=".."){ echo "dir"; } ?>">
				    <a class="folder-link" href="dialogo.php?<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>">
			...
			<?php }else{ ?>
				    ...
				    <div class="box">
					<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="dialogo.php?<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file; ?></a></h4>
				    </div>
...
?>
            ...
            ?>		
			<!------------------------------------------------------------Show us the thumbnail type for files------------------------------------------>
            <figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				<a href="javascript:void('IT'S 2')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $field_id; ?>" data-function="<?php echo $apply; ?>">
				<div class="img-precontainer">
				...
				<div class="box">				
				<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a href="javascript:void('IT'S 1 ')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $field_id; ?>" data-function="<?php echo $apply; ?>">
